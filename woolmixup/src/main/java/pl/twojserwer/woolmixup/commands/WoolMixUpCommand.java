...
public class WoolMixUpCommand implements CommandExecutor, TabCompleter {
    ...
    private void handleSetup(Player player, String[] args) {
        ...
        if (args.length < 2) {
            player.sendMessage("§eUzycie: /woolmixup setup <nazwa> [rozmiar=21]");
            return;
        }
        String name = args[1];
        int size = 21;
        ...
        if (size < MIN_SIZE || size > MAX_SIZE) {
            player.sendMessage("§cRozmiar musi byc miedzy " + MIN_SIZE + " a " + MAX_SIZE + ".");
            return;
        }
        ...
        Arena arena = new Arena(name, player.getWorld().getName(), minX, y, minZ, size);
        arenaManager.addArena(arena);

        player.sendMessage("§aStworzono arene '" + name + "' (" + size + "x" + size
                + ") na Twojej aktualnej pozycji. Zagraj: §f/woolmixup start " + name);
    }
    ...
    private void handleList(Player player) {
        ...
        player.sendMessage("§7Areny: §f" + String.join(", ", arenaManager.getArenas().keySet()));
    }
